...
window.LCalendar = (function() {
	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			this.triggerInput = this.trigger.querySelector('input');
			...
			//this.finishMobileTime
			//console.dir(this)
			//params.callback = this.finishMobileDate ? this.finishMobileDate : function(){};
			//console.log('回调函数',params.callback);
			this.bindEvent(this.type, params);
			/*添加监听事件*/

		},
		bindEvent: function(type, params) {
			var _self = this;
			//呼出日期插件
			...
			function dateCtrlInit() {
				...
				if(/^\d{4}\/\d{1,2}\/\d{1,2}$/.test(_self.triggerInput.value)) {
					rs = _self.triggerInput.value.match(/(^|\/)\d{1,4}/g);
					dateArr.yy = rs[0] - _self.minY;
					dateArr.mm = rs[1].replace(/\//g, "") - 1;
					dateArr.dd = rs[2].replace(/\//g, "") - 1;
				} else if(/^\d{4}-\d{1,2}-\d{1,2}$/.test(_self.triggerInput.value)) {
					rs = _self.triggerInput.value.match(/(^|-)\d{1,4}/g);
					dateArr.yy = rs[0] - _self.minY;
					dateArr.mm = rs[1].replace(/-/g, "") - 1;
					dateArr.dd = rs[2].replace(/-/g, "") - 1;
			...
			function ymCtrlInit() {
				...
				if(/^\d{4}-\d{1,2}$/.test(_self.triggerInput.value)) {
					rs = _self.triggerInput.value.match(/(^|-)\d{1,4}/g);
					dateArr.yy = rs[0] - _self.minY;
					dateArr.mm = rs[1].replace(/-/g, "") - 1;
				} else {
			...
			function dateTimeCtrlInit() {
				...
				if(/^\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}$/.test(_self.triggerInput.value)) {
					rs = _self.triggerInput.value.match(/(^|-|\s|:)\d{1,4}/g);
					dateArr.yy = rs[0] - _self.minY;
					dateArr.mm = rs[1].replace(/-/g, "") - 1;
					dateArr.dd = rs[2].replace(/-/g, "") - 1;
			...
			function timeCtrlInit() {
				...
				if(/^\d{2}:\d{2}$/.test(_self.triggerInput.value)) {
					rs = _self.triggerInput.value.match(/(^|:)\d{2}/g);
			...
			function setDateGearTooth() {
				...
				var date_yy = _self.gearDate ? _self.gearDate.querySelector(".date_yy") : null;
				if(date_yy == null) {
					return;
				}
				var itemStr = "";
			...
			function finishMobileDate(e) {
				...
				_self.triggerInput.value = (date_yy % passY + _self.minY) + "/" + date_mm + "/" + date_dd;
				closeMobileCalendar(e);
				params.callback(_self.triggerInput.value);
			...
			function finishMobileYM(e) {
				...
				var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_mm").getAttribute("val"))) + 1;
				date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
				_self.triggerInput.value = (date_yy % passY + _self.minY) + "-" + date_mm;
				closeMobileCalendar(e);
			}
			//日期时间确认
			function finishMobileDateTime(e) {
				...
				_self.triggerInput.value = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
				closeMobileCalendar(e);
			}
			//时间确认
			function finishMobileTime(e) {
				...
				var time_mm = parseInt(Math.round(_self.gearDate.querySelector(".time_mm").getAttribute("val")));
				time_mm = time_mm > 9 ? time_mm : '0' + time_mm;
				_self.triggerInput.value = (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
				closeMobileCalendar(e);
			}
...
app.directive('timePicker', function() {
	return {
		...
		template: [
			...
			'{{conf.name}}：',
			'<input type="text"  class="input-select" readonly="readonly" placeholder="请选择">',
			'<i class="icon-next"></i>',
			'</label>',
		].join(''),
		replace: true,
		link: function(scope, ele, attrs) {
			...
			ele[0].setAttribute('id', scope.conf.elementId);
			/*获取input值*/
			var inputDom = ele[0].querySelector('input');
			calendar.init({
				'trigger': "#" + scope.conf.elementId, //标签id
				'type': 'date', //date 调出日期选择 datetime 调出日期时间选择 time 调出时间选择 ym 调出年月选择,
				'minDate': '2010-01-01', //最小日期
				'maxDate': "2030-12-12", //最大日期
				'callback': function(value) {
					scope.conf.result(value);
				}
			});
			/*时间重置*/
			scope.conf.resetTime = function() {
				inputDom.value = '';
			}
		}
	}
